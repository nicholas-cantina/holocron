...
SPICE_LEVEL_PATTERN = re.compile(
    r"Profanity is always allowed and encouraged.")
CONVERSATION_STYLE_SHORT_PATTERN = re.compile(
    r"Keep messages short, under 15 words")
CONVERSATION_STYLE_MEDIUM_PATTERN = re.compile(
    r"Keep messages short. Do not exceed 45 words")
...
def extract_bot(conversation):
    ...
    user_name = re.search(CAPTURE_USER_NAME_PATTERN,
                          first_user_message_content).group(1).strip()
    full_name = re.search(CAPTURE_FULL_NAME_PATTERN,
                          first_user_message_content).group(1).strip()
    personality = re.search(CAPTURE_PERSONALITY_PATTERN,
                            first_user_message_content).group(1).strip()
    spice_level = "spicy" if re.search(
        SPICE_LEVEL_PATTERN, first_user_message_content) else "mild"
...
def extract_conversation(conversation):
    last_message = find_first(reversed(
        conversation["conversations"]), lambda message: message["role"] == "user")
    last_message_user_id = re.search(
        CAPTURE_OTHER_USER_NAME_PATTERN, last_message["content"]).group(1)
    return {
...
def extract_channel(conversation):
    ...
    for message in sent_messages:
        if message["role"] == "user":
            user_id = re.search(CAPTURE_OTHER_USER_NAME_PATTERN,
...
def extract_sent_messages(conversation):
    first_user_message_content = conversation["conversations"][1]["content"]
    bot_user_name = re.search(
        CAPTURE_USER_NAME_PATTERN, first_user_message_content).group(1).strip()
    bot_full_name = re.search(
        CAPTURE_FULL_NAME_PATTERN, first_user_message_content).group(1).strip()
